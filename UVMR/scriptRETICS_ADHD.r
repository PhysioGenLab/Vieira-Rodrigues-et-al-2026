...
exposure_dat <- read_exposure_data(
  filename = "reticulocyte_count_qc.csv",
...
# --- STEIGER CORRECTION ---
# Steiger requires outcome N sample size.
...
res <- mr(dat)
write.csv(res, "res_univariada_reticulocitos.csv", row.names=F)

het <- mr_heterogeneity(dat)
write.csv(het, "heterogeneidade_reticulocitos.csv", row.names=F)

pleio <- mr_pleiotropy_test(dat)
write.csv(pleio, "pleiotropia_reticulocitos.csv", row.names=F)
...
write.csv(dat, "dat_final_univariada_reticulocitos.csv", row.names=F)
...
cat(capture.output(print(raps_res)), file="mr_raps_reticulocitos_full.txt")
...
cat(capture.output(print(conmix_res)), file="mr_conmix_reticulocitos.txt")
...
try({
    ...
    write.csv(res_steiger, "res_reticulocitos_post_steiger.csv", row.names=F)
}, silent = FALSE)
