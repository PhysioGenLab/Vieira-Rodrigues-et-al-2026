...
d2 <- fread("TSP.sumstats.sumstats.gz") # Trait 2

# 4. Merge and Allele Alignment
print("Merging and Aligning Alleles...")
data <- ldscore_all %>%
  inner_join(d1, by = "SNP") %>%
  inner_join(d2, by = "SNP", suffix = c(".adhd", ".tsat"))

# Initial NA cleaning
data <- data %>% filter(!is.na(Z.adhd) & !is.na(Z.tsat) & !is.na(L2))

# --- ALLELE ALIGNMENT LOGIC ---
data <- data %>%
  mutate(Z.tsat.adj = case_when(
    A1.adhd == A1.tsat ~ Z.tsat,
    A1.adhd == A2.tsat ~ Z.tsat * -1,
    TRUE ~ as.numeric(NA)
  )) %>%
  filter(!is.na(Z.tsat.adj))
