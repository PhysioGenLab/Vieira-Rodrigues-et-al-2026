...
d2 <- fread("TIBC.sumstats_Leo2.sumstats.gz") # Trait 2

# 4. Merge and Allele Alignment
print("Merging and Aligning Alleles...")
data <- ldscore_all %>%
  inner_join(d1, by = "SNP") %>%
  inner_join(d2, by = "SNP", suffix = c(".adhd", ".tibc"))

# Initial NA cleaning
data <- data %>% filter(!is.na(Z.adhd) & !is.na(Z.tibc) & !is.na(L2))

# --- ALLELE ALIGNMENT LOGIC ---
data <- data %>%
  mutate(Z.tibc.adj = case_when(
    A1.adhd == A1.tibc ~ Z.tibc,
    A1.adhd == A2.tibc ~ Z.tibc * -1,
    TRUE ~ as.numeric(NA)
  )) %>%
  filter(!is.na(Z.tibc.adj))
