...
outcome_dat <- read_outcome_data(
  ...
  filename = "erythrocyte_count_qc.csv",
  sep = ",",
  snp_col = "SNP",
  beta_col = "BETA",
  se_col = "SE",
...
dat$samplesize.outcome[is.na(dat$samplesize.outcome)] <- 350475

# 5. MR and Diagnostics (Heterogeneity and Pleiotropia)
message("--- Running MR and Diagnostics ---")
res <- mr(dat)
write.csv(res, "res_univariada_ADHD_to_Eritrocitos.csv", row.names=F)

het <- mr_heterogeneity(dat)
write.csv(het, "heterogeneidade_ADHD_to_Eritrocitos.csv", row.names=F)

pleio <- mr_pleiotropy_test(dat)
write.csv(pleio, "pleiotropia_ADHD_to_Eritrocitos.csv", row.names=F)
...
write.csv(dat, "dat_final_univariada_ADHD_to_Eritrocitos.csv", row.names=F)
...
cat(capture.output(print(raps_res)), file="mr_raps_ADHD_to_Eritrocitos_full.txt")
...
cat(capture.output(print(conmix_res)), file="mr_conmix_ADHD_to_Eritrocitos.txt")
...
try({
    ...
    write.csv(res_steiger, "res_ADHD_to_Eritrocitos_post_steiger.csv", row.names=F)
}, silent = FALSE)
