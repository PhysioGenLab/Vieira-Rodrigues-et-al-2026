...
d2 <- fread("Hb_conc_munge.sumstats.gz") # Trait 2

# 4. Merge and Allele Alignment
print("Merging and Aligning Alleles...")
data <- ldscore_all %>%
  inner_join(d1, by = "SNP") %>%
  inner_join(d2, by = "SNP", suffix = c(".adhd", ".hb"))

# Initial NA cleaning
data <- data %>% filter(!is.na(Z.adhd) & !is.na(Z.hb) & !is.na(L2))

# --- ALLELE ALIGNMENT LOGIC ---
data <- data %>%
  mutate(Z.hb.adj = case_when(
    A1.adhd == A1.hb ~ Z.hb,
    A1.adhd == A2.hb ~ Z.hb * -1,
    TRUE ~ as.numeric(NA)
  )) %>%
  filter(!is.na(Z.hb.adj))
