...
d2 <- fread("erythrocyte_count.sumstats.gz") # Trait 2

# 4. Merge and Allele Alignment
print("Merging and Aligning Alleles...")
data <- ldscore_all %>%
  inner_join(d1, by = "SNP") %>%
  inner_join(d2, by = "SNP", suffix = c(".adhd", ".rbc"))

# Initial NA cleaning
data <- data %>% filter(!is.na(Z.adhd) & !is.na(Z.rbc) & !is.na(L2))

# --- ALLELE ALIGNMENT LOGIC ---
data <- data %>%
  mutate(Z.rbc.adj = case_when(
    A1.adhd == A1.rbc ~ Z.rbc,
    A1.adhd == A2.rbc ~ Z.rbc * -1,
    TRUE ~ as.numeric(NA)
  )) %>%
  filter(!is.na(Z.rbc.adj))
