...
res <- mr(dat)
write.csv(res, "res_univariada_ADHD_to_TIBC.csv", row.names=F)

het <- mr_heterogeneity(dat)
write.csv(het, "heterogeneidade_ADHD_to_TIBC.csv", row.names=F)

pleio <- mr_pleiotropy_test(dat)
write.csv(pleio, "pleiotropia_ADHD_to_TIBC.csv", row.names=F)
...
write.csv(dat, "dat_final_univariada_ADHD_to_TIBC.csv", row.names=F)
...
cat(capture.output(print(raps_res)), file="mr_raps_ADHD_to_TIBC_full.txt")
...
cat(capture.output(print(conmix_res)), file="mr_conmix_ADHD_to_TIBC.txt")
...
try({
    ...
    write.csv(res_steiger, "res_ADHD_to_TIBC_post_steiger.csv", row.names=F)
}, silent = FALSE)
