...
res <- mr(dat)
write.csv(res, "res_univariada_TSAT.csv", row.names=F)

het <- mr_heterogeneity(dat)
write.csv(het, "heterogeneidade_TSAT.csv", row.names=F)

pleio <- mr_pleiotropy_test(dat)
write.csv(pleio, "pleiotropia_TSAT.csv", row.names=F)
...
write.csv(dat, "dat_final_univariada_TSAT.csv", row.names=F)
...
cat(capture.output(print(raps_res)), file="mr_raps_TSAT_full.txt")
...
cat(capture.output(print(conmix_res)), file="mr_conmix_TSAT.txt")
...
try({
    ...
    write.csv(res_steiger, "res_TSAT_post_steiger.csv", row.names=F)
}, silent = FALSE)
